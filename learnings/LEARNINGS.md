...
## [LRN-20260907-001] 做统计 / 指标功能前，先验证源字段的真实取值分布——写入侧硬编码会让指标恒为常量

**Logged**: 2026-09-07 | **Status**: resolved | **Tags**: metrics, statistics, data-quality, verification

### Summary
新增「调用统计」（调用量 / 成功率 / 耗时）时，最容易漏的不是聚合 SQL，而是**源字段根本没被正确写入**：日志拦截里 `statusFlag` 被硬编码成常量 0，聚合逻辑再正确，成功率也恒为 0%。统计类需求的验收要前移到「字段有没有真值」。

### Details
- 场景：QCM V2 接口平台新增统计看板，按 `interface_api_log.statusFlag`（0 失败 / 1 成功 / 2 异常）算成功率。写完 SQL 回头核对写入侧，发现 `LogInterceptor` 里 `.statusFlag(0)` 是硬编码，全表无条件全写 0——成功率这个核心指标必然恒为 0。
- 为什么容易漏：需求是「统计」，注意力全在聚合与维度上；而字段的语义由注释/枚举定义（看起来是对的），真实取值分布却由**写入侧代码**决定，两者可能长期脱节且无人发现（因为此前没人消费这个字段）。
- 判定方法：动手写聚合前，先做一次「字段健康度检查」——① 全仓搜该字段的**写入点**（不是读取点）；② 看写入值是常量还是按结果计算；③ 有条件就按取值分组 count 一次看分布。任一步发现「唯一写入点是常量」，统计口径就得先修写入侧。
- 同类扩散：耗时字段（是否真算了差值）、来源字段（是否留空）、租户/应用标识（是否透传）同样适用。

### Suggested Action
1. 统计需求开工顺序固定为：**源字段取值分布 → 维度可关联 → 聚合 SQL**；跳过第一步的统计页不要交付。
2. 发现写入侧缺失时，把修复纳入本次范围并在交付说明单列一条「统计口径配套修复」，不要静默只改聚合逻辑。
3. 页面/接口上对「未配置 / 未知」维度值显式兜底显示（如「未配置」），避免把数据缺失伪装成 0 值指标。
