...
### 0.1 功能是「后台管理系统量级」时：分端而治，别指望一套代码通吃

钉钉 H5 微应用后台可配**两个首页地址**（移动端 + PC 端），这是天然双入口（钉钉小程序只有移动端，H5 才有 PC 端）。

| 端 | 定位 | 技术栈 |
|---|---|---|
| PC 端（钉钉 PC 客户端，**主战场**） | 全量后台功能：多列表格、批量操作、复杂表单、导入导出、权限配置 | Vue3 + Vite + Element Plus + 成熟后台脚手架 |
| 移动端（钉钉手机端） | 轻操作子集：待办审批、扫码、填报、查看、消息 | Vue3 + Vant 4 |

硬约束：**移动端没有成熟的表格组件**（多列 / 固定列 / 行内编辑 / 批量勾选在手机上体验崩坏）。把全量后台搬上手机不是适配问题，是交互形态问题。别做响应式后台，做两套入口。

PC 端是钉钉客户端内嵌 Chromium 内核，Element Plus、表格、导出均正常。

### 0.2 后台脚手架选型（PC 端底座）

| 脚手架 | 选它的场景 | 代价 |
|---|---|---|
| **RuoYi-Vue3（若依）** | 后端是 Java，要最快拿到「后台大部分功能」：用户 / 角色 / 菜单 / 部门 / 岗位 / 字典 / 登录与操作日志 / 定时任务 / 通知公告 + 代码生成 | 封装不深、样式偏传统；深度定制要啃它的代码生成模板 |
| vue-pure-admin | 已有后端，只缺前端底座；Element Plus，封装克制、文档全、示例多 | 系统管理基础模块（字典、日志、定时任务）要自己补 |
| vben-admin | 大团队长期演进、需要 UI 库可切换 | 抽象层级深、依赖重、Vite 冷启动慢、底层改造成本高 |

判定：后端 Java + 要「后台大部分功能」→ **RuoYi-Vue3** 起步最快；已有后端只缺前端 → vue-pure-admin。

**低代码备选：钉钉宜搭**——企业全在钉钉上、需求以表单 + 流程 + 报表为主、团队小，宜搭原生免登 / 组织架构 / 消息通知，1~2 天能出系统。天花板：复杂业务规则与大数据量表格，触顶后需迁出自研。

### 0.3 钉钉侧相对普通后台的四个差异

1. **免登替代账号密码**：`requestAuthCode` 拿 code → 后端换 userid → 映射自家用户 → 签发后台自己的 token；之后 RBAC 完全走后台逻辑（流程见第 2、3 节）。
2. **两个首页地址**：钉钉后台分别填移动端 / PC 端入口，两套前端构建产物（或同一 SPA 按 UA 分流），共用一套后端 API 与权限模型。
3. **下载与跳转**：容器内 `window.open` / a 标签下载不可靠，改后端返回文件流，或用 `dd.biz.util.openLink`。
4. **JSAPI 仅容器内可用**（扫一扫 / 选人 / 定位），PC 端与移动端行为不一致，需 `dd.env.platform` 分流 + 浏览器回退（见第 7.3 节）。

### 0.4 落地顺序与工期（后台量级）

1. 起 RuoYi-Vue3 前后端跑通本地 → 约 0.5 天
2. 后端加钉钉免登接口，替换登录入口 → 1~2 天（签名链路见第 2、4 节）
3. 钉钉后台建 H5 微应用，填 PC 端首页地址，PC 端跑通 → 0.5 天
4. 移动端 Vant 子应用，只做轻操作页 → 1~2 周（视页面量）
5. 真机联调 → 1~2 天（坑见第 9、10 节）

合计：可演示骨架 3~5 天，可用系统 2~3 周（取决于 CRUD 页面数量）。
